    ...
    <main class="main">
      ...
      <div class="stats">
        ...
        <div class="card">
          ...
          <div class="balance">
            <span class="amount">
              {{ Math.floor(krwBalance).toLocaleString('ko-KR') }}
            </span>
        ...
        <div class="card">
          ...
          <div class="balance">
            <span class="amount">
              {{ Math.floor(totalAssetsKrw).toLocaleString('ko-KR') }}
            </span>
            <span class="unit">원</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="balance">
            <span class="amount">
              {{ Math.floor(totalProfit).toLocaleString('ko-KR') }}
            </span>
            <span class="unit">원</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="balance">
            <span class="amount">
              {{ Math.floor(todayProfit).toLocaleString('ko-KR') }}
            </span>
            <span class="unit">원</span>
          </div>
        </div>
      ...
      <div class="actions">
        ...
        <div class="buttons">
          ...
          <template v-if="!isLoggedIn">
            ...
            <router-link class="btn success" to="/signup">
              회원가입
            </router-link>
          ...
          <template v-else>
            ...
            <button
              class="btn"
              :class="isRunning ? 'danger' : 'success'"
              @click="toggleBot"
            >
              {{ isRunning ? '봇 정지' : '봇 시작' }}
            </button>
      ...
      <div class="card logs">
        ...
        <div class="log-header">
          ...
          <button class="btn-text" @click="clearLogs">
            지우기
          </button>
...
export default {
  ...
  methods: {
    ...
    async fetchTodayProfit() {
      ...
      try {
        ...
        this.todayProfit = Number(res.data.todayRealizedPnl ?? 0)

      } catch (e) {

        this.addLog("오늘 수익 조회 실패", "error")
    ...
    async fetchTotalProfit() {
      ...
      try {
        ...
        this.totalProfit = Number(res.data.totalRealizedPnl ?? 0)

      } catch (e) {

        this.addLog("총 수익 조회 실패", "error")
    ...
    async fetchTotalAssets() {
      ...
      try {
        ...
        this.totalAssetsKrw = Number(res.data.totalAssetsKrw ?? 0)

      } catch (e) {

        this.addLog("총 자산 조회 실패", "error")
    ...
    async fetchStartId() {
      ...
      try {
        ...
        this.addLog("startId 조회 실패", "error")
    ...
    try {
      ...
      this.addLog("실시간 로그 조회 실패", "error")
  ...
  mounted() {
    ...
    if (!localStorage.getItem("accessToken")) {
      ...
      this.addLog("로그인이 필요합니다", "warning")
